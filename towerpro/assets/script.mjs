   ...
   function init() {
    ...
    canvas = document.getElementById(
     'canvas');
    w = canvas.width = window
     .innerWidth;
    h = canvas.height = window
     .innerHeight;
   ...
   function game() {
    ...
    temp = new Box(-stack_arr[len].w,
     stack_arr[len].y - box_h,
     stack_arr[len].w);
    temp.m = true;
    draw();
   }
   ...
   function canvas_update() {
    ...
    //when all good
    let temp_box = new Box(box_x,
     temp.y, box_w);
    stack_arr.push(temp_box);

    //another temp box
    let a = stack_arr[stack_arr
     .length - 1];
    let b;
