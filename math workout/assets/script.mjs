...
function findDivisor(x) {
 /*This function find the second largest divisor of a number. It's used to create multiplication expressions */
 let testing = x - 1;
 while (x % testing !== 0) {
  testing--;
 }
...
function integerToExpression(number,
 difficulty) {
 let randomizer = Math
  .random(); //This variable randomize the operation
 let randomValue = Math.round(Math
  .random() * (10 ** difficulty)
 ); // this variable randomize the number that will be used to create the expression
 if (randomizer < 0.2) {
  return number;
